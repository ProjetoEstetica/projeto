...
import br.com.estetica.clientesDAO.ComandosBancoCliente;
...
public class ClientesTela {
	...
	public void tableLoad() {
		try {
			...
			PreparedStatement pst = (PreparedStatement) con
					.prepareStatement("SELECT nome_completo FROM cadastro_clientes ORDER BY nome_completo ASC");
	...
	private void initialize() {
		...
		ComandosBancoCliente clienteDB = new ComandosBancoCliente();
		// info dos clientes
		...
		lblClientes.setBounds(361, 11, 115, 31);
		...
		tabela.getSelectionModel().addListSelectionListener(e -> {
			...
			if (tabela.getSelectedRow() != -1) {
				...
				try {
					Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/clientes",
							"root", "");
					PreparedStatement pst = (PreparedStatement) con
							.prepareStatement("SELECT * FROM cadastro_clientes WHERE nome_completo = ?");
	...
	private void filterTable() {
		try {
			...
			String query = "SELECT nome_completo FROM cadastro_clientes WHERE nome_completo LIKE ? ORDER BY nome_completo ASC";
			Connection con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/clientes", "root",
					"");
	...
	private JSONObject buscarCEP(String cep) {
		try {
			...
			if (conn.getResponseCode() != 200) {
				throw new RuntimeException("Erro ao consultar o CEP: " + conn.getResponseCode());
			}
